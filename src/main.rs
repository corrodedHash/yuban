...
fn complete_server(rocket: Rocket, static_file_path: PathBuf) -> Rocket {
    rocket
        ...
        .mount("/", routes::get_fix_routes())
        .mount(
            "/",
            rocket_contrib::serve::StaticFiles::from(static_file_path),
        )
}

fn api_server(rocket: Rocket) -> Rocket {
    rocket.mount("/api", routes::get_api_routes())
}
...
fn main() {
    ...
    let db = loop {
        ...
        let mysql_opts = mysql::OptsBuilder::new()
            ...
            .db_name(Some("yuban"))
            .pass(Some(
                std::env::var("YUBAN_MYSQL_PASSWORD")
                    .ok()
                    .unwrap_or_else(|| "PajqMDXIloNcwuxG27udp3gy4EBi".to_owned()),
            ));
    ...
    let r: Rocket = if cmdline_opts.complete {
        ...
        api_server(rocket::custom(config))
    };
